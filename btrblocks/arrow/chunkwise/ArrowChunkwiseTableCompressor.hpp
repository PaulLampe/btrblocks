...
struct ArrowChunkwiseTableCompressor {
 public:
  ArrowChunkwiseTableCompressor() = default;

  static std::shared_ptr< vector< tuple< OutputBlockStats, vector<u8> > > > compress(std::shared_ptr<arrow::Table> batch);

  static std::shared_ptr<arrow::Table> decompress(std::shared_ptr< vector< vector<u8> > >& compressedBatch);
};
